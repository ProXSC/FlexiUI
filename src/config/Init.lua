...
function ConfigManager:Init(WindowTable)
    if not WindowTable.Folder then
        warn("[ FlexiUI.ConfigManager ] Window.Folder is not specified.")
        return false
    end
...
function ConfigManager:SetPath(customPath)
    if not customPath then
        warn("[ FlexiUI.ConfigManager ] Custom path is not specified.")
        return false
    end
    ...
    if isfile(ConfigModule.Path) then
        ...
        if success and configData and configData.__autoload then
            ...
            task.spawn(function()
                ...
                if success then
                    if Window.Debug then print("[ FlexiUI.ConfigManager ] AutoLoaded config: " .. configFilename) end
                else
                    warn("[ FlexiUI.ConfigManager ] Failed to AutoLoad config: " .. configFilename .. " - " .. tostring(result))
                end
            end)
        end
    end
